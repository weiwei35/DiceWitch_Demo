Shader "UI/DiceWitch/Map Grid Reveal"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _ClipRect;
            float4 _RevealCircles[MAX_REVEAL_CIRCLES];
            float _RevealCount;
            ...
            fixed4 frag(v2f input) : SV_Target
            {
                ...
                [unroll]
                for (int i = 0; i < MAX_REVEAL_CIRCLES; i++)
                {
                    float4 circle = _RevealCircles[i];
                    float distanceToCenter = distance(localPosition, circle.xy);
